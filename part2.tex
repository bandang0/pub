...
\fig{shock}{0.5}{Cut across the shock front, showing the ejected matter, and the interstellar material accumulating at the forward shock front.}
...
As of now, we will fix the value of $\epsilon_e$ to the fiducial value of 0.1 (see e.g. \cite{41}). This value is supported by the fitting of synchrotron models such as ours to GRB as well as supernovae afterglows.
...
\fig{schema}{0.8}{The geometry of the remnant. A spherical shell-like shock front (or a part of a sphere) is expanding at speed $\beta(t)$, while every particle radiates in its own reference frame according to an emissivity $j\p$.}
...
\fig{jets}{0.8}{Geometrical configurations for fluxes from \it{(a)} a spherical remnant, \it{(b)} a jet seen on axis, \it{(c)} a jet seen off axis.}
...
We observe the agreement to be good on the parameter ranges of interest for our study case, and we conclude that our calculations are sufficient for our purposes. Note that full-fledged calculations, albeit available, would not suite our purposes as they are slow and not fit for parameter space exploration, as we will perform in the next section.
...
\fig{jetbest}{0.7}{3 GHz best-fit light curves for the mono-kinetic jet afterglow model (blue, see Section 5.2), and the monokinetic spherical shell model (orange, see Section 5.3). None are an acceptable fit. The difference between both best-fit models is apparent only in the decreasing phase, where the jet break implies a steeper decrease due to the additional $\left(\Gamma \theta_j\right)^2 \propto \tobs^{-3/4}$ factor in the flux.}
...
The best-fit jet afterglow has parameters $n = 3 \times 10^{-4}$ cm\sp{--3}, $E_0 = 2 \times 10^52$ erg, $\Gamma_0 = 4$, $\espilon_B = 3 \times 10^{-3}$, $\thetaobs = \theta_j = 6$ deg.
...
The best-fit mono-kinetic spherical model has parameters $n = 2 \times 10^{-5}$ cm\sp{--3}, $E_0 = 3 \times 10^51$, $\Gamma_0 = 5$, $\epsilon_B = 4 \times 10^{-3}$.
...
\fig{radial}{0.7}{The 3~GHz best-fit light curve for the synchrotron emission model with a radially structured outflow. This model has parameters as reported in Table \ref{cocoon}}
...
\fig{galac}{0.8}{\it{Left:} Localisation of the merger site as projected on the sky plane in NGC4993. \it{Right:} Best-fit Sersic profile subtracted image of the NGC4993. Notice the shells indicating a recent galactic merger event supporting a non-trivial density profile (\cite{33}).}
...
Before we proceed, it is interesting to establish a scale for $E_j$, a scale which will in the future allow us to compare merger event jet afterglows with GRB energetics.

Which estimate for $E_j$? The fluence of GRB170817A is $(2.08\pm0.2)\times10^{-7}$~erg~cm\sp{-2}, and the isotropic equivalent dissipated energy is $E^{\rm{iso}}_\gamma$~$6 \times 10^{47}$~erg. In the standard vision where the GRB was produced by a relativistic jet, this is the total kinetic energy of the jet which was dissipated in gamma rays. The kinetic energy of the jet producing the afterglow is thus the remainder of this kinetic energy. By denoting $E^K_0$ the initial kinetic energy of the jet, we thus have:
...
\fig{ejthetaj}{0.7}{Constraints on the intrinsic parameters of the relativistic jet, as visualized in the $(\theta_j, E_j)$ plane for various values of $\theta_{\rm{obs}}$ below $\thetaobs^{\rm{max}} = 28\deg$. Indicative energy scales corresponding to different gamma efficiencies for GR170817A are also present.}
...
\fig{jet_theta}{0.7}{Afterglow light curves of on-axis observers for a hidden jet. Observe the jet-break in the jet light curves which radically change the slopes.}
...
\fig{bump}{0.7}{A jet afterglow which complies to our hiding condition (the maximum is weaker than the data at the time of maximum), yet the afterglow is clearly apparent.}
